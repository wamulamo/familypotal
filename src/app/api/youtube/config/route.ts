...
import { fetchVideoIdsByChannelId, fetchVideoIdsByPlaylistId } from "@/lib/youtube-api";
...
export async function GET() {
  ...
  // 並び順: 個別URLの登録順 → チャンネルごと（各チャンネルは新しい順）→ プレイリストごと（各プレイリスト内の順）
  const video_ids = Array.from(
    new Set([...videoIdsFromUrls, ...channelVideoIds, ...playlistVideoIds])
  );
